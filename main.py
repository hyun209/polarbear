...
with tab1:
    rows=[]
    for s in env_data:
        if s not in growth_data: continue
        e=env_data[s]; g=growth_data[s]
        rows.append({"학교":s,"온도":e["temperature"].mean(),"습도":e["humidity"].mean(),
                     "pH":e["ph"].mean(),"EC":e["ec"].mean(),"생중량":g["생중량(g)"].mean()})
    df=pd.DataFrame(rows)

    fig=make_subplots(rows=2,cols=2,subplot_titles=["온도-생중량","습도-생중량","pH-생중량","EC-생중량"])
    vars=["온도","습도","pH","EC"]
    for i,v in enumerate(vars):
        r,c=divmod(i,2)
        fig.add_trace(go.Scatter(x=df[v],y=df["생중량"],mode="markers+text",text=df["학교"]),row=r+1,col=c+1)
